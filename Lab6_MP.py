...
koefs = [0.3, 4.1, 2.8, 7.8, 9.7, 0.6, 4.4, 3.4, 1.4, 0.2, 2.4]
...
def main(m, n):
    ...
    if n == d:
        n = 8 if n == 4 else 14
        print(f"n=dПроводимо експеримент знову з n = {n} та m = {m}")
        return main(m=m, n=n)
    if n == 14:
        print(
    ...
    if f_p > get_value(f_t, f_3)[f_4]:
        n = 8 if n == 4 else 14 
#       ця умова збільшує значення n з 4 до 8, або з 8 до 14, якщо за критерієм Фішера математична модель не є адекватною.
#       Збільшуючи n ми проводимо екперимент знову, доки модель не буде адекватною.
#       В даному випадку я не використовую цю умову, тому що з самого початку n = 14, це зроблено тому, що в іншому випадку модель завжди буде адекватною
        print(
            f"fp = {f_p} > ft = {get_value(f_t, f_3)[f_4]}.\n"
            f"Математична модель не адекватна\n"
            f"Проводимо експеримент знову з m = {m} та n = {n}")
        return main(m=m, n=n)
    else:
        print(
            f"fP = {f_p} < fT = {get_value(f_t, f_3)[f_4]}.\n"
